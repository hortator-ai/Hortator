...
## BUG-001: k9s not in PATH after webi install
- **Context:** `webi` installs k9s to `~/.local/bin` but that's not in root's default PATH
- **Fix applied:** Added `/etc/profile.d/local-bin.sh` to persist PATH
- **Suggestion:** Add `~/.local/bin` to PATH in cloud-init user_data, or install k9s via apt/binary download to `/usr/local/bin`

## BUG-002: Helm chart CRDs vs standalone CRDs
- **Context:** CRDs exist in both `crds/` (standalone yamls) and `charts/hortator/templates/crds.yaml`. Had to `--set crds.install=false` and apply standalone CRDs separately to avoid conflicts.
- **Suggestion:** Pick one canonical source. Recommended: standalone `crds/` dir + `crds.install=false` default, or use Helm's `crds/` directory convention (not templates).

## BUG-003: Example YAML uses wrong apiVersion
...
## BUG-004: Example ClusterAgentRole fields don't match CRD schema
...
## BUG-005: Example AgentTask uses `roleRef` / `retainPVC` — not in CRD
...
## BUG-007: Controller doesn't inject API key from model.apiKeyRef into agent pod env
...
## BUG-008: Agent image hardcoded as `:latest` in controller (3 places)
...
## BUG-009: No StorageClass shipped or documented for RKE2
...
## BUG-010: CRD schema / Go types mismatch on SecretKeyRef
